...
class KalmanFilter(object):
    ...
    def __init__(
        ...
        if not callable(transition_matrix):
        ...
        self._process_covariance_estimate = self._to_matrix(
    ...
    def predict(self, measurements, control_state=None):
        ...
        predicted_state_estimate = \
            self._transition_matrix * self._state_estimate
        ...
        predicted_covariance_estimate = (
            self._transition_matrix * self._process_covariance_estimate
        ) * numpy.transpose(self._transition_matrix) + self._process_error_estimate

        if len(measurements) > 0:
            measurements = numpy.array([measurements]).transpose()
        innovation = \
            measurements - self._observation_matrix * predicted_state_estimate
        innovation_covariance = \
            self._observation_matrix \
            * predicted_covariance_estimate \
            * numpy.transpose(self._observation_matrix) \
            + self._measurement_error_estimate

        kalman_gain = predicted_covariance_estimate \
            * numpy.transpose(self._observation_matrix) \
        ...
        self._process_covariance_estimate = (
            numpy.eye(size) - kalman_gain * self._observation_matrix
        ) * predicted_covariance_estimate
    ...
    def _to_matrix(vector_or_matrix):
        """Converts vectors to matrices, and leaves matrices the hell alone."""
