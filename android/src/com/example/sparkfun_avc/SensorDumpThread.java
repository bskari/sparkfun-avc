...
public class SensorDumpThread extends Thread {
	...
	public SensorDumpThread(DatagramSocket datagramSocket,
			LocationManager manager, String bestProvider,
	...
	public SensorDumpThread(DatagramSocket datagramSocket,
		...
		this.manager.requestLocationUpdates(LocationManager.GPS_PROVIDER,
				frequencyMilliseconds, 0, listener);
	}
		...
		while (!datagramSocket.isClosed() && running) {
			...
			if (location != null) {
				try {
					...
					json = String.format("{\"type\":\"telemetry\", "
							+ "\"latitude\": %4.9f," + "\"longitude\": %4.9f,"
							+ "\"bearing\": %4.9f" + "}", latitude, longitude,
							bearing);
				} catch (Exception e) {
					// I guess, send this to the server?
					json = e.toString();
				}
